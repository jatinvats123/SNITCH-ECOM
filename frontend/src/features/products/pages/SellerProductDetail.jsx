...
const CURRENCIES = ["INR","USD","EUR","GBP","JPY","AUD","CAD","CHF","CNY","SEK","NZD","MXN","SGD","HKD","NOK","KRW","TRY","RUB","BRL","ZAR"];

const currencySymbol = (c) => ({ INR:'₹', USD:'$', EUR:'€', GBP:'£', JPY:'¥', AUD:'A$', CAD:'C$' }[c] ?? c);
...
const SellerProductDetail = () => {
    ...
    const { handleGetProductById, handleCreateVariant, handleUpdateVariantStock, handleDeleteVariant } = useProduct();
    ...
    const removeAttribute = (i) => setForm(f => ({ ...f, attributes: f.attributes.filter((_, idx) => idx !== i) }));
    ...
    const handleSubmit = async (e) => {
        ...
        const attrMap = {};
        form.attributes.forEach(({ key, value }) => { if (key.trim()) attrMap[key.trim()] = value.trim(); });
    ...
    const saveStock = async (variantId) => {
        ...
        try {
            ...
            await fetchProduct();
            setStockEdits(s => { const n = { ...s }; delete n[variantId]; return n; });
        } catch (err) {
            console.error(err);
        } finally {
    ...
    if (!product) return (
        <div className="min-h-screen bg-[#f5f1ea] flex flex-col items-center justify-center gap-6">
            ...
            <button onClick={() => navigate('/seller/dashboard')} className="text-xs uppercase tracking-[0.3em] underline underline-offset-4 text-[#8a7a64] hover:text-[#1f1b17]">
                Back to Dashboard
            </button>
        </div>
    );

    const mainImage = product.images?.[0]?.url || 'https://placehold.co/400x500/ede7db/a09080/webp?text=';
                ...
                <div className="flex h-full flex-col justify-between p-8">
                    ...
                    <div>
                        ...
                        <div className="rounded-3xl border border-black/5 bg-white/70 p-5 shadow-[0_18px_50px_rgba(31,27,23,0.06)]">
                            ...
                            <p className="mt-3 text-sm leading-6 text-[#5d5448]">Manage variants, control stock, and keep listings sharp.</p>
                        </div>
                    </div>
                </div>
            </aside>
            ...
            <main className="relative md:ml-72">
                <div className="mx-auto max-w-7xl px-5 py-8 sm:px-8 md:px-10 md:py-10 lg:px-12 lg:py-12">
                    ...
                    <header className="mb-10 flex flex-col gap-8 rounded-4xl border border-black/5 bg-white/75 p-6 shadow-[0_24px_80px_rgba(31,27,23,0.06)] backdrop-blur-xl sm:p-8 lg:flex-row lg:items-start lg:p-10">
                        ...
                        <div className="w-full flex-shrink-0 overflow-hidden rounded-2xl bg-[#ede7db] lg:w-52">
                            <div className="aspect-[3/4] w-full">
                                <img src={mainImage} alt={product.title} className="h-full w-full object-cover object-center" />
                            </div>
                        </div>
                        ...
                        <div className="flex flex-1 flex-col justify-between gap-6">
                            <div>
                                ...
                                <h1 className="mt-3 text-3xl font-semibold tracking-tight sm:text-4xl">{product.title}</h1>
                            ...
                            <div className="flex flex-wrap gap-6">
                                <div className="rounded-2xl border border-black/5 bg-[#f7f3ec] px-5 py-4">
                                    ...
                                    <p className="mt-1.5 text-xl font-semibold tracking-tight">
                                        {currencySymbol(product.price?.currency)}{product.price?.amount?.toLocaleString()}
                                    </p>
                                </div>
                                <div className="rounded-2xl border border-black/5 bg-[#f7f3ec] px-5 py-4">
                                    <p className="text-[0.65rem] uppercase tracking-[0.28em] text-[#8a7a64]">Variants</p>
                                    <p className="mt-1.5 text-xl font-semibold tracking-tight">{product.variants?.length || 0}</p>
                                </div>
                                <div className="rounded-2xl border border-black/5 bg-[#f7f3ec] px-5 py-4">
                    ...
                    <section className="rounded-4xl border border-black/5 bg-white/75 p-6 shadow-[0_24px_80px_rgba(31,27,23,0.06)] backdrop-blur-xl sm:p-8 lg:p-10">
                        ...
                        {/* ── Create variant form ── */}
                        {showForm && (
                            <form onSubmit={handleSubmit} className="mb-10 rounded-[1.75rem] border border-dashed border-[#c9b89a]/60 bg-[#fbf8f3] p-6 sm:p-8">
                                <p className="mb-6 text-[0.7rem] uppercase tracking-[0.35em] text-[#8a7a64]">New Variant</p>

                                <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                                    {/* Price amount */}
                                    <div className="flex flex-col gap-2">
                                        <label className="text-[0.7rem] uppercase tracking-[0.28em] text-[#8a7a64]">Price Amount</label>
                                        <input
                                            type="number"
                                            min="0"
                                            required
                                    ...
                                    {/* Currency */}
                                    <div className="flex flex-col gap-2">
                                        <label className="text-[0.7rem] uppercase tracking-[0.28em] text-[#8a7a64]">Currency</label>
                                        <select
                                    ...
                                    {/* Stock */}
                                    <div className="flex flex-col gap-2">
                                        <label className="text-[0.7rem] uppercase tracking-[0.28em] text-[#8a7a64]">Initial Stock</label>
                                        <input
                                            type="number"
                                            min="0"
                                            value={form.stock}
                                    ...
                                    <div className="flex flex-col gap-2 sm:col-span-2">
                                        <label className="text-[0.7rem] uppercase tracking-[0.28em] text-[#8a7a64]">Variant Images <span className="normal-case text-[#b5a898]">(up to 7 files)</span></label>
                                        <input
                                            type="file"
                                            multiple
                                            accept="image/*"
                                            ...
                                            <div className="flex flex-wrap gap-2 mt-3">
                                                {form.images.map((file, idx) => (
                                                    <div key={idx} className="relative group">
                                                        ...
                                                        <button
                                                            type="button"
                                                            onClick={() => setFormField('images', form.images.filter((_, i) => i !== idx))}
                                ...
                                <div className="mt-8">
                                    <div className="mb-4 flex items-center justify-between">
                                        <p className="text-[0.7rem] uppercase tracking-[0.28em] text-[#8a7a64]">Attributes <span className="normal-case">(e.g. Color, Size)</span></p>
                                        <button type="button" onClick={addAttribute} className="text-[0.7rem] uppercase tracking-[0.25em] text-[#8a7a64] hover:text-[#1f1b17] transition-colors">
                                            + Add
                                        </button>
                                    </div>
                                ...
                                <div className="mt-8 flex justify-end">
                                    ...
                                    >
                                        {creating ? (
                                            <><span className="h-3.5 w-3.5 rounded-full border border-[#f7f3ec]/40 border-t-[#f7f3ec] animate-spin" />Creating…</>
                                        ) : 'Create Variant'}
                                    </button>
                                </div>
                            </form>
                        ...
                        {product.variants && product.variants.length > 0 ? (
                            <div className="flex flex-col gap-4">
                                {product.variants.map((variant, idx) => {
                                    ...
                                    return (
                                        <div key={variant._id || idx} className="flex flex-col gap-5 rounded-[1.75rem] border border-black/5 bg-[#fbf8f3] p-5 sm:flex-row sm:items-start sm:p-6">

                                            {/* Variant image */}
                                            <div className="h-20 w-20 flex-shrink-0 overflow-hidden rounded-xl bg-[#ede7db]">
                                                {variantImage
                                                    ? <img src={variantImage} alt="variant" className="h-full w-full object-cover" />
                                                    : <div className="flex h-full w-full items-center justify-center text-[#c9b89a] text-xs">—</div>
                                                }
                                            </div>
                                            ...
                                            <div className="flex flex-1 flex-col gap-4">
                                                ...
                                                {attrs.length > 0 ? (
                                                    <div className="flex flex-wrap gap-2">
                                                        {attrs.map(([k, v]) => (
                                                            <span key={k} className="rounded-full border border-black/8 bg-white px-3 py-1 text-xs text-[#5d5448]">
                                                ...
                                                <div className="flex flex-wrap items-end gap-6">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="mt-1 text-lg font-semibold tracking-tight">
                                                            {currencySymbol(variant.price?.currency)}{variant.price?.amount?.toLocaleString()}
                                                        </p>
                                                    </div>
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="mt-1 flex items-center gap-2">
                                                            <input
                                                                ...
                                                                min="0"
                                                                value={stockDraft}
                                                                onChange={e => setStockEdits(s => ({ ...s, [variant._id]: e.target.value }))}
                            ...
                            !showForm && (
                                <div className="rounded-[1.75rem] border border-dashed border-black/10 bg-[#fbf8f3] px-6 py-16 text-center sm:px-10">
                                    <p className="text-sm uppercase tracking-[0.3em] text-[#8a7a64]">No variants yet</p>
                                    <h3 className="mt-4 text-2xl font-semibold tracking-tight">Add your first variant</h3>
                                    <p className="mx-auto mt-3 max-w-md text-sm leading-7 text-[#6d6357]">
                                        Create variants to offer different sizes, colours or configurations — each with its own price and stock.
                                    </p>
                                    <button
