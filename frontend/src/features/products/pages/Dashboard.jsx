...
const Dashboard = () => {
  ...
  const handleDeleteListing = async (productId, title) => {
    const confirmed = window.confirm(
      `Delete ${title}? This will remove the listing from your store.`,
    );
    if (!confirmed) return;
            ...
            <nav className="space-y-1">
              {navigationItems.map((item) => (
                ...
                >
                  <span>{item}</span>
                  {item === activeTab ? (
                    <span className="h-2 w-2 rounded-full bg-[#d8c39a]" />
                  ) : null}
                </button>
              ))}
            </nav>
          ...
          <header className="mb-10 flex flex-col gap-6 rounded-4xl border border-black/5 bg-white/75 p-6 shadow-[0_24px_80px_rgba(31,27,23,0.06)] backdrop-blur-xl sm:p-8 lg:flex-row lg:items-end lg:justify-between lg:p-10">
            <div className="max-w-2xl">
              ...
              <p className="mt-4 max-w-xl text-base leading-7 text-[#6d6357] sm:text-lg">
                Track products, review listings, and manage the store with more space and less
                noise.
              </p>
            </div>
              ...
              {stats.map((stat) => (
                ...
                >
                  <div className="mb-8 flex items-center justify-between">
                    ...
                    <span className="text-xs uppercase tracking-[0.25em] text-[#8a7a64]">
                      {activeTab}
                    </span>
                  </div>
                  <p className="text-sm uppercase tracking-[0.24em] text-[#8a7a64]">{stat.label}</p>
                  <h3 className="mt-3 text-4xl font-semibold tracking-tight text-[#1f1b17]">
                    {stat.value}
                  </h3>
                </div>
              ))}
              ...
              {recentProducts.length > 0 ? (
                <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
                  {recentProducts.map((product) => (
                    ...
                    >
                      ...
                      <div className="p-4">
                        <h3 className="truncate text-sm font-medium text-[#1f1b17]">
                          {product.title}
                        </h3>
                ...
                <div className="rounded-[1.75rem] border border-dashed border-black/10 bg-[#fbf8f3] px-6 py-16 text-center sm:px-10">
                  ...
                  <p className="mx-auto mt-3 max-w-md text-base leading-7 text-[#6d6357]">
                    Add a product to start building out the catalog and make this dashboard come
                    alive.
                  </p>
                </div>
              )}
                    ...
                    >
                      ...
                      <div className="space-y-4 p-5 sm:p-6">
                        <div className="space-y-1">
                          <h3 className="truncate text-lg font-medium text-[#1f1b17]">
                            {product.title}
                          </h3>
          ...
          {activeTab === "Analytics" && (
            <section className="rounded-4xl border border-black/5 bg-white/75 p-6 shadow-[0_24px_80px_rgba(31,27,23,0.06)] backdrop-blur-xl sm:p-8 lg:p-10">
              ...
              <div className="rounded-[1.75rem] border border-dashed border-black/10 bg-[#fbf8f3] px-6 py-16 text-center sm:px-10">
                ...
                <h3 className="mt-4 text-2xl font-semibold tracking-tight text-[#1f1b17]">
                  Trends will appear once you have sales
                </h3>
                <p className="mx-auto mt-3 max-w-md text-base leading-7 text-[#6d6357]">
                  Revenue, top products, and traffic trends will show up here once orders start
                  coming in. Your current catalog snapshot is above.
                </p>
              </div>
            </section>
          )}
